...
def a_star_search(initial_state, goal):
    open_list = [(0, 0, initial_state, [])]  # Priority queue with (total_cost, actual_cost, state, path)
    closed_set = set()
    ...
    while open_list:
        ...
        for child in find_children(current_state):
            child_actual_cost = actual_cost + 1  # Assuming uniform cost for simplicity
...
def main():
    ...
    **** έναρξη αναζήτησης
    ...
    Astar_steps=find_solution(make_front(initial_state), make_queue(initial_state), [], goal, Astarmethod)

    if bfs_steps > dfs_steps:
        print("DFS BETTER STEPS: ",dfs_steps)
    else:
        print("BFS BETTER STEPS: ",bfs_steps)    
